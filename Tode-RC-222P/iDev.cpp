...
DBINITAAL(("OnOff::OnOff(TodeIndex,RFID,Hardware)"),(_TodeIndex),(_RFID))
    ...
    if (IsLocal) {
      if ( _Hardware==0 ) { DBERRORL(("OnOff::OnOff IsLocal but Hardware==0")) return; } 
        ...
        IOPinMode = SubList->Add(new DevPinMode("pinMode",_RFID,1,IOPin));
        SubList->Add(new MenuName("Del Device", NAVDELDEV));
    }
}
//-----------------------------------------------------------------------------------------------------
int OnOff::IOValue() {                                                 
...
byte Press::Loop() {
  ...
  if (NavSelected == SEL_SETNAME){ DBINFOL(("Press::Loop() - NavSelected == SEL_SETNAME")) return 0; }
  if (!OnDisplay()) { DBINFOL(("Press::Loop() !OnDisplay()")) return 0; }
  int Ave = IOValue();
...
DBINITAAL(("Dist::Dist(TodeIndex,RFID,Hardware)"),(_TodeIndex),(_RFID))
    ...
    if (IsLocal) {
      ...
      TrigPinMode = SubList->Add(new DevPinMode ("TrigPMode", _RFID,1,TrigPin));
      EchoPin =     SubList->Add(new PinSelect  ("Echo",      _RFID,2,_Hardware));
      EchoPinMode = SubList->Add(new DevPinMode ("EchoPMode", _RFID,3,EchoPin));
      SubList->Add(new MenuName("Del Device", NAVDELDEV));
    }

  TrigPinMode->SetPinMode();
  EchoPinMode->SetPinMode();
}
//-----------------------------------------------------------------------------------------------------
...
DBINITAAL(("STSTP3W::STSTP3W(TodeIndex,RFID,Hardware)"),(_TodeIndex),(_RFID))
  ...
  if (IsLocal) {
    ...
    StartPin =      SubList->Add(new PinSelect  ("StartPin",  _RFID,0,_Hardware));
    StartPinMode =  SubList->Add(new DevPinMode ("StartPMode",_RFID,1,StartPin));
    StopPin =       SubList->Add(new PinSelect  ("StopPin",   _RFID,2,_Hardware));
    StopPinMode =   SubList->Add(new DevPinMode ("StopPMode", _RFID,3,StopPin));
    StatusPin =     SubList->Add(new PinSelect  ("Status",    _RFID,4,_Hardware));
    StatusPinMode = SubList->Add(new DevPinMode ("StatPMode", _RFID,5,StatusPin));
                    SubList->Add(new MenuName   ("Del Device", NAVDELDEV));
  }
  StartPinMode->SetPinMode();
  StopPinMode->SetPinMode();
  StatusPinMode->SetPinMode();
}
//-----------------------------------------------------------------------------------------------------
