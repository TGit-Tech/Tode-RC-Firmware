...
DBENTERL(("Dist::ApplySettings()"))
  SettingsValid=true;
  EPin = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+0);
  EPinMode = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+1);
...
DBENTERL(("Dist::ApplySettings()"))
  SettingsValid=true;
  trigPin = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+0);
  trigPinmode = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+1);
  echoPin = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+2);
  echoPinmode = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+3);
  ESamplesToGet = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+4);

  if (!ApplyPinSettings(trigPin,trigPinmode)) SettingsValid = false;
  if (!ApplyPinSettings(echoPin,echoPinmode)) SettingsValid = false;
  
  // Reset if new SamplesToGet
  if ( Samples != 0 ) {
    if ( Samples->SamplesToGet != ESamplesToGet ) { delete Samples; Samples=0; }
  }
...
DBENTERL(("AnaInput::ApplySettings()"))
  ...
  EPin = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+0);
  EPinMode = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+1);
  EEPROM.get(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+2, PreAddValue);
  EEPROM.get(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+4, MultNumValue);
  EEPROM.get(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+6, MultDenValue);
  EEPROM.get(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+8, AddValue);
  EEPROM.get(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+10, ESamplesToGet);
...
byte AnaInput::Loop() {
  ...
  if (!OnDisplay()) { DBINFOL(("AnaInput::Loop() !OnDisplay()")) return 0; }
  int Ave = IOValue();
  if ( Ave == LastGetIOValue ) return 0;
  LastGetIOValue = Ave;
  DisplayValue();
...
DBENTERL(("STSTP3W::ApplySettings()"))
  SettingsValid=true;
  startPin =      EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+0);
  startPinMode =  EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+1);
  stopPin =       EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+2);
  stopPinMode =   EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+3);
  statusPin =     EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+4);
  statusPinMode = EEPROM.read(EMC_DEVSETTINGS+(RFID*AEB_DEVSETTINGS)+5);

  if (!ApplyPinSettings(startPin,startPinMode)) SettingsValid = false;
  if (!ApplyPinSettings(stopPin,stopPinMode)) SettingsValid = false;
  if (!ApplyPinSettings(statusPin,statusPinMode)) SettingsValid = false;
...
DBENTERAL(("STSTP3W::IOValue[SET]: "),(_Value))
  ...
  if (_Value==0) {
    DBINFOAL(("STSTP3W::IOValue[SET] digitalWrite(stopPin,0)"),(stopPin))
    digitalWrite(stopPin,0);
    LastStopMS = millis();
    
  } else if (_Value==1) {
    DBINFOAL(("STSTP3W::IOValue[SET] digitalWrite(startPin,0)"),(startPin))
...
DBENTERL(("STSTP3W::IOValue[GET]"))
  if ( !SettingsValid ) { DBERRORL(("STSTP3W::IOValue[GET] !SettingsValid")) return -1; }
...
byte STSTP3W::Loop() {
  ...
  if ( LastStartMS!=0 ) {
    ...
    if ( (millis() - LastStartMS)>=3000 ) {
      ...
      digitalWrite(startPin,1); LastStartMS=0; LastStopMS=0;
