...
DBENTERL(("MenuName::BuildSubList()"))
  //SubList = new MenuList(F("MN Setup"));
...
//#####################################################################################################################
MenuNameSet::MenuNameSet(int _EENameAddress):
MenuName(_EENameAddress) {
  DBINITAL(("MenuNameSet::MenuNameSet(EENameAddress)"),(_EENameAddress))
  EENameAddress=_EENameAddress;
  //DisplayNameSet();
}
//.....................................................................................................
byte MenuNameSet::Navigate(byte _Key) {                 DBENTERAL(("MenuNameSet::Navigate(key):"),(_Key,HEX))
  ...
  if ( _Key == NAVEXITNAMESET ) {
    DBINFOL(("MenuNameSet::Navigate Key == NAVEXITNAMESET"))
    NavSelected=SEL_NAME; 
...
//-----------------------------------------------------------------------------------------------------
byte MenuNameSet::NavName(byte _Key) {                         
DBENTERAL(("MenuNameSet::NavName"),(_Key,HEX))
  // Processes a SET or OKAY
  if ( _Key == NAVKEYSET ) { 
    ...
    if ( SubList != 0 ) { DBINFOL(("MenuNameSet::NavName NAVGOTOSUBLIST")) return NAVGOTOSUBLIST; }
    if ( ReturnKey != NAVKEYNONE ) return ReturnKey;
  }
  return _Key;
}
//-----------------------------------------------------------------------------------------------------
void MenuNameSet::DisplayName(byte _ID) {
//DBENTERAL(("MenuName::DisplayName"),(Name()))
  if ( Display==0 ) { DBERRORL(("MenuNameSet::DisplayName Display==0")) return; }
  Display->SETCURSOR(XPos,YPos);
  if ( NavSelected==SEL_NAME ) { Display->BGNAMESELT; Display->TXTSELT; }
  else { Display->BGNAMENORM; Display->TXTNORM; }
  if (_ID!=BNONE) Display->print(_ID);
...
//-----------------------------------------------------------------------------------------------------
byte MenuNameSet::NavNameSet(byte _Key) {                   
DBENTERAL(("MenuNameSet::NavNameSet(Key)"),(_Key,HEX))
...
//-----------------------------------------------------------------------------------------------------
void MenuNameSet::DisplayNameSet() {
DBENTERL(("MenuNameSet::DisplayNameSet()"))
  if ( Display==0 ) { DBERRORL(("MenuNameSet::DisplayNameSet Display==0")) return 0; }
...
//-----------------------------------------------------------------------------------------------------
void MenuNameSet::DspSetToName(bool _Selected) {
...
//-----------------------------------------------------------------------------------------------------
void MenuNameSet::DspChar(bool _Selected) {
...
DBENTERL(("MenuValue::DisplayValue()"))
  ...
  if ( NavSelected==SEL_VALUE ) { 
    if      ( yStatus==STSRFGETTING ) { Display->TXTRFGETTING; Display->BGVALSELSTS; }
    else if ( yStatus==STSRFGOT )     { Display->TXTRFGOT; Display->BGVALSELSTS; }
    else if ( yStatus==STSRFGETFAIL ) { Display->TXTRFGETFAIL; Display->BGVALSELSTS; }
    else                              { Display->BGVALSELT; Display->TXTSELT; }
  } else { 
    if      ( yStatus==STSRFGETTING ) { Display->TXTRFGETTING;  Display->BGVALNORM; }
    else if ( yStatus==STSRFGOT )     { Display->TXTRFGOT;  Display->BGVALNORM; }
    else if ( yStatus==STSRFGETFAIL ) { Display->TXTRFGETFAIL;  Display->BGVALNORM; }
    else                              { Display->TXTNORM; Display->BGVALNORM; }
  }

  // Status Colors
  if      ( yStatus==STSRFGETTING ) { Display->TXTRFGETTING; }
  else if ( yStatus==STSRFGOT )     { Display->TXTRFGOT; }
  else if ( yStatus==STSRFGETFAIL ) { Display->TXTRFGETFAIL; }
  else if ( yStatus==STSEXPIRED )   { Display->TXTEXPIRED; }
...
//-----------------------------------------------------------------------------------------------------
void MenuValue::Status(byte _Status) { 
  yStatus = _Status; 
