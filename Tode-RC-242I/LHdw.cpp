...
DBENTERAL(("Tode::NewDevice(_DTKey)"),(_DTKey,HEX))
  ...
  EEPROM.update(EA,_DTKey);EEPROM.update(EA+1,_RFID);                 // Save DT-Key and RFID in EEPROM
/*
  if ( _DTKey==DT_RW_ONOFF )          {EEPROM.put(EA+2,"?ONOFFO?"); }
  else if ( _DTKey==DT_RO_ONOFF )     {EEPROM.put(EA+2,"?ONOFFI?"); }
  else if ( _DTKey==DT_RO_DIST )      {EEPROM.put(EA+2,"?DIST?");   }
  else if ( _DTKey==DT_RW_STSTP3W )   {EEPROM.put(EA+2,"?STSPT3W?");}
  else if ( _DTKey==DT_RO_ANAINPUT )  {EEPROM.put(EA+2,"?ANAIN?");  }
  else if ( _DTKey==DT_DC_SETPOINT )  {EEPROM.put(EA+2,"?SETPT?");  }
  else if ( _DTKey==DT_DC_MATH )      {EEPROM.put(EA+2,"?MATH?");   }
  else                                {EEPROM.put(EA+2,"?NAME?");   }
  */
  Version(Version()+1);
  
  return AddDevice(_DTKey, _RFID, true);
  
}
//-----------------------------------------------------------------------------------------------------
MenuItem* Tode::AddDevice(byte _DTKey, byte _RFID, bool _NewDevice) { 
DBENTERAAL(("Tode::AddDevice(DTKey,RFID) = "),(_DTKey,HEX),(_RFID))
  ...
  if ( _DTKey == DT_RW_ONOFF ) {
    //OnOff(byte _TodeIndex, byte _RFID, bool _ReadOnly = false)
    Devices[_RFID] = this->Add(new OnOff(TodeIndex,_RFID,_NewDevice,false));
    return Devices[_RFID];
  } else if ( _DTKey == DT_RO_ONOFF ) {
    Devices[_RFID] = this->Add(new OnOff(TodeIndex,_RFID,_NewDevice,true));
    return Devices[_RFID];
  } else if ( _DTKey == DT_RO_DIST ) { 
    Devices[_RFID] = this->Add(new Dist(TodeIndex,_RFID,_NewDevice));
    return Devices[_RFID];    
  } else if ( _DTKey == DT_RW_STSTP3W ) {
    Devices[_RFID] = this->Add(new STSTP3W(TodeIndex,_RFID,_NewDevice));
    return Devices[_RFID];
  } else if ( _DTKey == DT_RO_ANAINPUT ) {
    Devices[_RFID] = this->Add(new AnaInput(TodeIndex,_RFID,_NewDevice));
    return Devices[_RFID];
  } else if ( _DTKey == DT_DC_SETPOINT ) {
    
    //DCLimits::DCLimits(byte _TodeIndex, byte _RFID, HdwSelect* _Hardware=0)
    Devices[_RFID] = this->Add(new DCSetPoint(TodeIndex,_RFID));
    return Devices[_RFID];
  } else if ( _DTKey == DT_DC_MATH ) {
    Devices[_RFID] = this->Add(new DCMath(TodeIndex,_RFID));
    return Devices[_RFID];
  } else if ( _DTKey == DT_RW_ANAOUTPUT ) {
    Devices[_RFID] = this->Add(new AnaOutput(TodeIndex,_RFID));
    return Devices[_RFID];
  }
  return 0;
