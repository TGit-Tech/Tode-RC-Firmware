...
#define RXVALEXPIREMS 15000    // 15-Seconds till remote value expires
...
/******************************************************************************************************************//**
 * @class   MenuNameSet
 * @brief   Setup-SubList **ITEM** for EEPROM MenuName changing via the User Interface.
 *********************************************************************************************************************/
class MenuNameSet : public MenuName {
  public:
    MenuNameSet(int _EENameAddress);
    

  protected:
    virtual byte          Navigate(byte _Key) override;                   ///< Keypad Navigate by \ref KEY
    virtual byte          NavName(byte _Key) override;                    ///< NAME Navigate 
    virtual byte          NavNameSet(byte _Key);                          ///< NAME SET Navigate 
    virtual void          DisplayName(byte _ID=BNONE) override;           ///< Display the Row-Name
...
class MenuValue : public MenuName {
  public:
    ...
    virtual void          Status(byte _Status) override;                                  ///< SET status calls DisplayValue()
    int                   VMin = -9999;
    int                   VMax = 9999;                      ///< -32,766 to 32,767 (but full per digit set)
