...
//-----------------------------------------------------------------------------------------------------
void Ebyte::FixedTransmission(bool _Fixed) {    DBENTERAL(("Ebyte::FixedTransmission[SET](?)"),(_Fixed))
  if (EM<=EM_UNKNOWN) {DBERRORL(("Ebyte::GetParam() EM<=EM_UNKNOWN")) return; }
  byte CurrentMode=Mode();                                        // Check Radio is Identified
  if (CurrentMode!=ERFSETMODE) Mode(ERFSETMODE);                  // Radio in SETMODE
  if ( EParamGotBytes==0 ) GetParam();                            // Get Parameters if needed.
...
//-----------------------------------------------------------------------------------------------------
bool Ebyte::FixedTransmission() {                         DBENTERL(("Ebyte::FixedTransmission[GET]()"))
  if ( EParamGotBytes==0 ) GetParam();                                    // Get Parameters if needed
  if (EMS==EMS_E220E22)   {return ((paramREG3&0x40)>>6);}                 //E220&E22 REG3.6
  else if (EMS==EMS_E32)  {return ((paramREG1OPT&0x80)>>7);}              //E32 OPT.7
  else {DBERRORL(("Ebyte::FixedTransmission[GET]() EMS not set!"))}
}
//-----------------------------------------------------------------------------------------------------
...
void Ebyte::E32FECOn(bool _On) {
  ...
  if (EMS==EMS_E32) {
    if (_On) {paramREG1OPT += (1<<2);} else {paramREG1OPT += (0<<2);}  //E32 OPT.2
  }
  SetParam();                                                     // Write Parameters
  Mode(CurrentMode);                                              // Return to Current Mode
}
      ...
      if (ReplyOk) {
        if (EMS==EMS_E220E22) {
          ...
          switch(EParamGotBytes) {          //b[0]=0xC0
            ...
            case 5: paramREG1OPT=b; break;  //b[5]=OPTION FIX.7,IODR.6,WAKE.543,FEC.2,TX.10
          }
        }
...
void Ebyte::CheckRadio() {                                              DBENTERL(("Ebyte::CheckRadio"))
  ...
  if ( EM==EM_NOTGOT ) {
    ...
    if ( Pinfo[0]==0xC3 ) { // E32 1-byte HEAD
      if        ( Pinfo[1]==0x32 ) {  // Is 433MHz
        ...
        if (Pinfo[2]==0x53&&Pinfo[3]==0x1E){EM=EM_E32_433T30D;EMS=EMS_E32;DBINFOL(("Ebyte::CheckRadio FOUND EM=EM_E32_433T30D [1B]"))} //SN:W020902S07862
  ...
  if (EM==EM_NOTGOT) {
    ...
    // FOUND RADIO:  Check Radio Register Settings
    ...
    // Check Radio-RFAddress matches EEPROM-RFAddress
    ...
    // Check UART ( FOUND EM=EM_E32_433T30D [1B] Ebyte::CheckRadio UARTSped=?(3) )
    ...
    bool fixedtrans = FixedTransmission(); // This here GET why is it calling SetParam?
    if (!fixedtrans) {
      DBINFOL(("Ebyte::CheckRadio FixedTransmission=false SET FixedTransmission(true)"))
      FixedTransmission(true);
    } else {;DBINFOL(("Ebyte::CheckRadio FixedTransmission=true"))}

    if (EMS==EMS_E32) { 
      bool FECon = E32FECOn();
      if (!FECon) {
        DBINFOL(("Ebyte::CheckRadio FECon=false SET E32FECOn(true)"))
        E32FECOn(true);
      } else {;DBINFOL(("Ebyte::CheckRadio FECon=true"))}
    }
  }
  Mode(ERFNORMMODE);
}
