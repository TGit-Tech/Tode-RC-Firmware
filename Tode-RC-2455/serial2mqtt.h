...
class serial2mqtt
{
  ...
  void publish(templateTopic topic, templateMessage message, int QoS=0, bool Retained=false)
  {
    if(_debugEnabled)
    {
    ...
    if(_useJSONobjects)
    {
      ...
      _serialStream->print(F("\",\"qos\":"));
      _serialStream->print(QoS);
      _serialStream->print(F(",\"retained\":"));
      if (Retained) {_serialStream->print(F("true"));} else {_serialStream->print(F("false"));}
  ...
  private:
    ...
    uint32_t  _loopbackInterval = 195e2;   //19.5s loopback to stop Pi ~/serial2mqtt from disconnecting serial port.
    bool      _online = false;
